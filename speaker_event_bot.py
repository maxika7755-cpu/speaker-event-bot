...
async def module_speakers(callback: CallbackQuery) -> None:
    ...
    for ms in links:
        ...
        kb.button(text=f"🗑 {ms.speaker.full_name}", callback_data=f"speaker_delete_ask:{ms.id}")
...
def build_schedule_matrix_from_words(words: list[dict[str, Any]], page_width: float) -> str:
    ...
    lines = [
        ...
        "Do NOT extend an event through empty cells automatically.",
        "Merge adjacent rows only when the next non-empty text is a clear continuation of the same title, speaker, or description.",
        "A new activity title starts a new schedule item.",
        "",
                ...
                input=[
                    {
                        ...
                        "content": (
                            ...
                            "Не продлевай событие через пустые строки автоматически. Не объединяй разные активности в один огромный блок. "
                            "Если появляется новое название активности, создай новый item. "
        ...
        for day, cell_text in row["cells"].items():
            ...
            should_continue = bool(
                ...
                and (
                    ...
                    or (cell_text[:1].islower() and row["start"] == current.get("end_time"))
                )
            )
            if should_continue:
