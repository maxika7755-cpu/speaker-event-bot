...
def reminder_min_local(tz: ZoneInfo) -> datetime:
    ...
    min_local = (now_local + timedelta(minutes=1)).replace(second=0, microsecond=0)
    if min_local <= now_local:
        min_local += timedelta(minutes=1)
    return min_local
...
async def quick_reminder_when(message: Message, state: FSMContext) -> None:
    ...
    if not when:
        await message.answer("Не понял дату и время. Пример: 24.07 14:30")
        return
    selected_local = when.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz)
    min_local = reminder_min_local(tz)
    if selected_local < min_local:
        await message.answer(
            f"Ошибка: время уже прошло. Выберите минимум {min_local.strftime('%d.%m %H:%M')}."
        )
        return
