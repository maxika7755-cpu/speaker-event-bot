...
ACCESS_PERMISSIONS = [
    ...
    ("speakers", "Спикеры"),
    ("schedule", "Расписание"),
    ("schedule_upload", "Загрузка сетки"),
    ("schedule_edit", "Правка слотов"),
    ("ai", "ИИ"),
    ("program_manage", "Управление программой"),
    ("payments", "Оплаты и документы"),
    ("flowers", "Цветы"),
    ("quick_reminder", "Напоминалка"),
    ("reminders", "Напоминания"),
    ("broadcast", "Рассылка"),
    ("settings", "Настройки"),
]
DEFAULT_GRANT_PERMISSIONS = ["modules", "speakers", "schedule", "flowers", "quick_reminder"]
PHOTO_REPORT_MESSAGES = [
    "📸 Фото для отчета само себя не сделает. Камеру в руки, момент в архив!",
    "📷 Напоминание: если не сфоткали, значит для отчета этого почти не было.",
    "📷 Поймайте пару живых кадров для отчета. Потом все скажут спасибо, особенно будущие мы.",
    "📸 Фото-чек! Улыбки, зал, спикеры, движ - все в копилку отчета.",
    "🎞 Пора пополнить фотоархив. Без паники, просто красиво щелкнуть пару раз.",
    "📷 Отчет просит еды. Покормите его фотографиями.",
    "📸 Не забываем фото: люди работают, модуль живет, отчет потом сияет.",
    "🧾 Фото для отчета, пожалуйста. Бюрократия любит доказательства, но мы сделаем их красивыми.",
    "📷 Часовой фото-звоночек: поймайте атмосферу, пока она не убежала в кофе-брейк.",
    "📸 Сделайте пару кадров. Это не просто фото, это будущая легенда в папке 'отчет финал финал'.",
]
...
class ModuleSpeaker(Base):
    ...
    flower_excluded: Mapped[bool] = mapped_column(Boolean, default=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class AppSetting(Base):
    ...
    performance_reminders_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    docs_reminders_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    flowers_reminders_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    team_reminders_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    ...
    last_greeting_date: Mapped[date | None] = mapped_column(Date, nullable=True)
...
class BotUserActivity(Base):
    __tablename__ = "bot_user_activity"

    user_id: Mapped[int] = mapped_column(Integer, primary_key=True)
    username: Mapped[str] = mapped_column(String(128), default="", index=True)
    full_name: Mapped[str] = mapped_column(String(255), default="")
    last_action: Mapped[str] = mapped_column(String(255), default="")
...
def user_display_name(user: Any) -> str:
    ...
    return " ".join(part for part in parts if part).strip() or getattr(user, "username", "") or str(getattr(user, "id", ""))


async def track_user_activity(user: Any, event: Any) -> None:
    action = ""
    if isinstance(event, CallbackQuery):
        action = f"button:{event.data or ''}"
    elif isinstance(event, Message):
        text = (event.text or event.caption or "").strip()
        action = text[:120] if text else "message"
    action = action[:255]
    try:
        async with SessionLocal() as session:
            row = await session.get(BotUserActivity, user.id)
            now = datetime.utcnow()
            if not row:
                row = BotUserActivity(user_id=user.id, first_seen_at=now)
                session.add(row)
            row.username = normalize_username(getattr(user, "username", "") or "")
            row.full_name = user_display_name(user)
            row.last_action = action
            row.last_seen_at = now
            await session.commit()
    except Exception as exc:
        log.warning("cannot track user activity: %s", exc)
...
class AccessMiddleware(BaseMiddleware):
    async def __call__(self, handler: Any, event: Any, data: dict[str, Any]) -> Any:
        ...
        if await access_allowed(user.id, user.username):
            await track_user_activity(user, event)
            await maybe_send_daily_greeting(event, user.id, getattr(user, "first_name", "") or "коллега")
            if isinstance(event, CallbackQuery) and not await callback_allowed(user.id, user.username, event.data or ""):
                ...
                try:
                    permissions = await effective_permissions(user.id, user.username)
                    await event.message.edit_text("Главное меню", reply_markup=kb_main(permissions).as_markup())
                except Exception as exc:
                    log.warning("cannot refresh menu after denied callback: %s", exc)
                return None
            return await handler(event, data)
...
async def maybe_send_daily_greeting(event: Any, user_id: int, first_name: str) -> None:
    today = datetime.now(ZoneInfo(DEFAULT_TZ)).date()
    async with SessionLocal() as session:
        settings = await get_settings(session, user_id)
        if settings.last_greeting_date == today:
            return
        settings.last_greeting_date = today
        await session.commit()
    text = (
        f"Привет, {html_escape(first_name)}! Бот МНМ на связи.\n\n"
        "Я тут, чтобы расписание не расползалось, спикеры не терялись, а цветы доезжали к тем, кому надо. "
        "Поработаем сегодня?"
    )
    try:
        if isinstance(event, Message):
        ...
        log.warning("daily greeting failed: %s", exc)
...
class QuickReminderCreate(StatesGroup):
    text = State()
    when = State()
...
class ScheduleSlotEdit(StatesGroup):
    title = State()
    time = State()
    setup = State()
    speakers = State()
...
def parse_reminder_time(value: str) -> time | None:
    value = (value or "").strip()
    matches = list(re.finditer(r"\b(\d{1,2})([:.])(\d{2})\b", value))
    if not matches:
        return None
    for match in reversed(matches):
        h, minute = int(match.group(1)), int(match.group(3))
        if not (0 <= h <= 23 and 0 <= minute <= 59):
            continue
        if match.group(2) == ".":
            date_only = re.fullmatch(r"\d{1,2}\.\d{1,2}(?:\.\d{2,4})?", value)
            if date_only and match.start() == 0:
                continue
        return time(h, minute)
    return None
...
def parse_date_value(value: str, fallback_year: int | None = None) -> date | None:
    ...
    for fmt in ("%Y-%m-%d", "%d.%m.%Y", "%d.%m.%y", "%d.%m"):
        try:
            ...
            pass
    dot_match = re.search(r"\b(\d{1,2})\.(\d{1,2})(?:\.(\d{2,4}))?\b", value)
    if dot_match:
        day_num = int(dot_match.group(1))
        month_num = int(dot_match.group(2))
...
def parse_reminder_datetime(text: str, timezone: str = DEFAULT_TZ) -> datetime | None:
    text = (text or "").strip()
    tm = parse_reminder_time(text)
    day = parse_date_value(text)
    if not tm or not day:
        return None
    tz = ZoneInfo(timezone or DEFAULT_TZ)
    return datetime.combine(day, tm).replace(tzinfo=tz).astimezone(ZoneInfo("UTC")).replace(tzinfo=None)


def reminder_min_local(tz: ZoneInfo) -> datetime:
    now_local = datetime.now(tz)
    return (now_local.replace(second=0, microsecond=0) + timedelta(minutes=1))


def display_program_day_label(image: "PrettyScheduleImage") -> str:
    if image.schedule_date:
        return image.schedule_date.strftime("%d.%m.%Y")
    return image.day_label
...
def clean_speaker_name(name: str) -> str:
    ...
    if not candidate or low in NON_SPEAKER_WORDS:
        return ""
    if any(marker in low for marker in NON_SPEAKER_MARKERS):
        return ""
    if len(words) == 1 and len(candidate) >= 3 and candidate[:1].isupper():
        return candidate
    if len(candidate) < 5:
        return ""
    return candidate
...
def module_speaker_needs_flowers(ms: ModuleSpeaker | None) -> bool:
    if not ms or ms.flower_excluded or not ms.speaker:
        return False
    return bool(ms.flower_required or ms.speaker.gender.lower().startswith("жен"))
...
def callback_permission_key(callback_data: str) -> str | None:
    ...
    if data.startswith("module_ai_question:") or data == "ai_module_menu":
        return "ai"
    if data.startswith("schedule_now:"):
        return None
    ...
    if data in {"speakers"} or data.startswith(("speaker:", "speaker_create:", "speaker_edit_", "speaker_field:", "speaker_delete", "module_speakers:", "module_speakers_delete_all", "speakers_delete_all")):
        return "speakers"
    if data.startswith(("pretty_schedule_upload", "pretty_schedule_delete")):
        return "program_manage"
    if data.startswith("upload_schedule:"):
        return "schedule_upload"
    if data.startswith(("schedule_slot_edit_title:", "schedule_slot_edit_time:", "schedule_slot_edit_setup:", "schedule_slot_edit_speakers:")):
        return "schedule_edit"
    if data == "schedule_all" or data == "program" or data.startswith(("schedule_", "pretty_schedule", "program_")):
        return "schedule"
    if data == "payments_all" or data.startswith(("module_payments:", "docs:", "doc_toggle:")):
        return "payments"
    if data == "flowers_all" or data.startswith(("module_flowers:", "module_flowers_disable:")):
        return "flowers"
    if data.startswith("quick_reminder"):
        return "quick_reminder"
    if data == "reminders" or data.startswith(("reminders_page:", "team_reminders_toggle")):
        return None
    if data.startswith(("recreate_reminders:", "user_reminder_delete")):
        return "reminders"
    if data == "broadcast" or data.startswith("broadcast_"):
        return "broadcast"
    if data == "bot_users" or data.startswith("access_") or data.startswith("settings") or data == "settings":
        return "settings"
    return None
...
def kb_main(permissions: set[str] | None = None) -> InlineKeyboardBuilder:
    ...
    items = [
        ...
        ("👤 Спикеры", "speakers"),
        ("▶️ Что идет сейчас?", "schedule_now:0"),
        ("📅 Расписание", "schedule_all"),
        ("Программа", "program"),
        ("🤖 Спросить ИИ", "ai_module_menu"),
        ("⏰ Напоминалка", "quick_reminder"),
        ("📣 Рассылка", "broadcast"),
...
def kb_error_back() -> InlineKeyboardBuilder:
    kb = InlineKeyboardBuilder()
...
async def find_speaker(session: AsyncSession, user_id: int, name: str) -> Speaker | None:
    ...
    requested_tokens = [token for token in re.findall(r"[a-zа-яё]+", name_norm, flags=re.I) if len(token) > 1]
    for sp in speakers:
        speaker_norm = re.sub(r"\s+", " ", sp.full_name.strip().lower())
        if name_norm == speaker_norm:
            return sp
    for sp in speakers:
        speaker_norm = re.sub(r"\s+", " ", sp.full_name.strip().lower())
        if len(name_norm) >= 6 and (name_norm in speaker_norm or speaker_norm in name_norm):
            return sp
    if len(requested_tokens) >= 2:
        requested_tail = requested_tokens[-1]
        for sp in speakers:
            speaker_tokens = [token for token in re.findall(r"[a-zа-яё]+", sp.full_name.lower(), flags=re.I) if len(token) > 1]
            overlap = set(requested_tokens) & set(speaker_tokens)
            same_last_name = requested_tail in speaker_tokens[1:] or requested_tail == (speaker_tokens[-1] if speaker_tokens else "")
            if same_last_name and len(overlap) >= 2:
                return sp
    return None


def parse_slot_speaker_input(raw: str) -> list[str]:
    if raw.strip() == "-":
        return []
    names: list[str] = []
    for part in re.split(r"[,;\n]+", raw):
        value = re.sub(r"\s*[-–]\s*уточнить.*$", "", part.strip(), flags=re.I)
        if not value:
            continue
        words = re.findall(r"[A-Za-zА-Яа-яЁё]+", value)
        if len(words) > 4 and value[:1].islower():
            continue
        if len(words) > 6:
            value = " ".join(words[:3])
...
async def get_schedule_slot_group(
    session: AsyncSession,
    item_id: int,
    owner_ids: list[int],
) -> tuple[ScheduleItem | None, list[ScheduleItem]]:
    item = await session.scalar(
        select(ScheduleItem)
        .join(Module)
        .where(ScheduleItem.id == item_id, Module.owner_user_id.in_(owner_ids))
    ...
    group = (
        await session.scalars(
            select(ScheduleItem)
            .where(
                ScheduleItem.module_id == item.module_id,
                ScheduleItem.date == item.date,
                ScheduleItem.start_time == item.start_time,
                ScheduleItem.end_time == item.end_time,
                ScheduleItem.title == item.title,
                ScheduleItem.location == item.location,
                ScheduleItem.format == item.format,
                ScheduleItem.comment == item.comment,
            )
            .options(selectinload(ScheduleItem.speaker), selectinload(ScheduleItem.module))
...
async def schedule_slot_card(
    session: AsyncSession,
    item: ScheduleItem,
    group_items: list[ScheduleItem],
    ...
    slot_setup = sorted({setup for row in group_items for setup in (row.setup or [])})
    speaker_setup = sorted({setup for ms in module_speakers for setup in (ms.setup or [])})
    setup_items = sorted(set(slot_setup + speaker_setup))
    lines = [
        f"📅 <b>{item.date:%d.%m.%Y}</b>",
        f"⏰ <b>{time_label(item.start_time)}-{time_label(item.end_time)}</b>",
        f"📁 Модуль: {html_escape(item.module.name if item.module else '')}",
        "",
        f"Тема: {html_escape(item.title or 'не указана')}",
    ]
    if speaker_names:
        lines.append("🎤 Спикеры: " + ", ".join(f"<b>{html_escape(name)}</b>" for name in speaker_names))
    if setup_items:
        lines.append("Сетап: " + html_escape(", ".join(setup_items)))
    if item.location:
        lines.append(f"Зал: {html_escape(item.location)}")
    if item.format:
        lines.append(f"Формат: {html_escape(item.format)}")
    if item.comment:
        lines.append(f"Комментарий: {html_escape(item.comment)}")
    kb = InlineKeyboardBuilder()
    if permission_allows(permissions, f"schedule_slot_edit_title:{item.id}"):
        kb.button(text="✏️ Изменить название", callback_data=f"schedule_slot_edit_title:{item.id}")
        kb.button(text="⏰ Изменить время", callback_data=f"schedule_slot_edit_time:{item.id}")
        kb.button(text="⚙️ Изменить сетап", callback_data=f"schedule_slot_edit_setup:{item.id}")
        kb.button(text="🎤 Изменить спикеров", callback_data=f"schedule_slot_edit_speakers:{item.id}")
    kb.button(text="⬅️ К расписанию дня", callback_data=f"schedule_day:{item.module_id}:{item.date.isoformat()}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    return "\n".join(lines), kb
...
async def module_card(
    ...
    flowers = [x for x in links if module_speaker_needs_flowers(x)]
    ...
    if permission_allows(permissions, f"module_payments:{module_id}"):
        lines.append(f"📄 <b>Документы закрыты:</b> {avg_progress}%")
    if upcoming:
        ...
        lines += [
            ...
            f"{time_label(upcoming.start_time)}-{time_label(upcoming.end_time)}",
        ]
        if upcoming.speaker:
            lines.append(f"🎤 <b>{html_escape(upcoming.speaker.full_name)}</b>")
        lines.append(f"Тема: {html_escape(upcoming.title or (ms.topic if ms else ''))}")
        if ms and ms.setup:
            lines.append(f"Сетап: {html_escape(', '.join(ms.setup))}")
...
async def speaker_card(session: AsyncSession, ms_id: int) -> tuple[str, InlineKeyboardBuilder]:
    ...
    if ms.setup:
        lines += ["", "⚙️ <b>Сетап:</b>"]
        lines += [f"- {html_escape(x)}" for x in ms.setup]
...
def actor_mention(user: Any) -> str:
    name = html_escape(getattr(user, "full_name", None) or getattr(user, "first_name", None) or "пользователь")
    return f'<a href="tg://user?id={user.id}">{name}</a>'


async def notify_program_change(bot: Bot, module: Module, actor: Any, action: str, day_label: str) -> None:
    text = (
        f"<b>Программа обновлена</b>\n"
        f"Модуль: {html_escape(module.name)}\n"
        f"День: {html_escape(day_label)}\n"
        f"Действие: {html_escape(action)}\n"
        f"Кто: {actor_mention(actor)}"
    )
    recipient_ids = {module.owner_user_id}
    async with SessionLocal() as session:
        grants = (
            await session.scalars(
                select(AccessGrant).where(
                    AccessGrant.owner_user_id == module.owner_user_id,
                    AccessGrant.allowed_user_id.is_not(None),
                )
            )
        ).all()
        recipient_ids.update(grant.allowed_user_id for grant in grants if grant.allowed_user_id)
    for recipient_id in recipient_ids:
        try:
            await bot.send_message(recipient_id, text)
        except Exception as exc:
            log.warning("program change notification failed to %s: %s", recipient_id, exc)
...
async def build_module_ai_context(session: AsyncSession, module_id: int) -> str:
    ...
    if links:
        for ms in links:
            ...
            flower = "да" if module_speaker_needs_flowers(ms) else "нет"
            lines.extend(
                [
...
async def start(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        module_ids = (
            await session.scalars(select(Module.id).where(Module.owner_user_id.in_(owner_ids)))
        ).all()
        for module_id in module_ids:
            await recreate_reminders_for_module(session, module_id, message.from_user.id)
        await session.commit()
    await message.answer(
        "Привет! Я на связи и готов помочь с рабочими задачами по модулю. Выберите нужный раздел в меню ниже.",
...
async def module_ai_question_answer(message: Message, state: FSMContext, bot: Bot) -> None:
    data = await state.get_data()
    ...
    async with SessionLocal() as session:
        ...
        if not module or module.owner_user_id not in owner_ids:
            ...
            return
        if message.from_user.id != module.owner_user_id:
            username = f"@{message.from_user.username}" if message.from_user.username else "username не указан"
            request_text = "\n".join([
                "🤖 <b>Запрос к ИИ-помощнику</b>",
                "",
                f"Кто: {actor_mention(message.from_user)}",
                ...
                "",
                "ИИ будет вызван. Это уведомление для контроля токенов.",
            ])
            try:
                await bot.send_message(module.owner_user_id, request_text)
            except Exception as exc:
                log.warning("cannot forward AI question to owner %s: %s", module.owner_user_id, exc)
...
async def module_edit_save(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if module:
            ...
            await session.commit()
            permissions = await access_permissions(session, message.from_user.id, message.from_user.username)
            text, kb = await module_card(session, module.id, permissions)
        else:
    ...
    async with SessionLocal() as session:
        ...
        modules = (
            await session.scalars(
                ...
                .order_by(Module.start_date.desc(), Module.name)
            )
        ).all()
        await session.commit()
    kb = InlineKeyboardBuilder()
    for module in modules:
        dates = module.dates_label or (module.start_date.strftime("%d.%m.%Y") if module.start_date else "даты не указаны")
        kb.button(text=f"📁 {module.name} | {dates}", callback_data=f"module_speakers:{module.id}")
    if modules:
        kb.button(text="🗑 Удалить всех моих спикеров", callback_data="speakers_delete_all_ask")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "👤 <b>Спикеры</b>\n\nСначала выберите модуль." if modules else "Модулей пока нет.", kb)
...
async def module_speakers(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        module = await session.scalar(
            select(Module).where(Module.id == module_id, Module.owner_user_id.in_(owner_ids))
        )
        if not module:
            await callback.answer("Модуль не найден или недоступен.", show_alert=True)
            return
        links = (
            await session.scalars(
                select(ModuleSpeaker)
    ...
    kb.adjust(1)
    title = f"👤 <b>Спикеры модуля</b>\n📁 {html_escape(module.name)}"
    if not links:
        title += "\n\nВ модуле пока нет спикеров."
    await send_or_edit(callback, title, kb)
...
async def docs_open(callback: CallbackQuery) -> None:
    ...
    kb.button(text="⬅️ К оплатам модуля", callback_data=f"module_payments:{ms.module_id}")
...
async def payments(callback: CallbackQuery) -> None:
    if callback.data == "payments_all":
        ...
        kb = InlineKeyboardBuilder()
        for module in modules:
            kb.button(text=f"{module.name} | {module.dates_label or date_range_label(module)}", callback_data=f"module_payments:{module.id}")
        kb.button(text="🏠 Главное меню", callback_data="menu")
        kb.adjust(1)
        await send_or_edit(callback, "💰 <b>Выберите модуль для оплат и документов</b>" if modules else "Модулей пока нет.", kb)
        return
    module_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
    ...
    kb.button(text="⬅️ К выбору модуля", callback_data="payments_all")
    kb.button(text="⬅️ К модулю", callback_data=f"module:{module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "\n".join(lines) if links else "Платных спикеров в этом модуле пока нет.", kb)


@router.callback_query(F.data.startswith("module_flowers:") | (F.data == "flowers_all"))
async def flowers(callback: CallbackQuery) -> None:
    if callback.data == "flowers_all":
        async with SessionLocal() as session:
            owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
            modules = (
                await session.scalars(
        ...
        kb = InlineKeyboardBuilder()
        for module in modules:
            kb.button(text=f"{module.name} | {module.dates_label or date_range_label(module)}", callback_data=f"module_flowers:{module.id}")
        kb.button(text="🏠 Главное меню", callback_data="menu")
        kb.adjust(1)
        await send_or_edit(callback, "🌸 <b>Выберите модуль для цветов</b>" if modules else "Модулей пока нет.", kb)
        return
    module_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        ...
        if module_id:
            stmt = stmt.where(ModuleSpeaker.module_id == module_id)
        links = [x for x in (await session.scalars(stmt)).all() if module_speaker_needs_flowers(x)]
        rows = []
        for ms in links:
            item = await session.scalar(
                select(ScheduleItem)
    ...
    kb = InlineKeyboardBuilder()
    for ms, _ in rows:
        kb.button(text=f"🚫 Цветы не нужны: {ms.speaker.full_name}", callback_data=f"module_flowers_disable:{ms.id}")
    if module_id:
        kb.button(text="⬅️ К модулю", callback_data=f"module:{module_id}")
        kb.button(text="🏠 Главное меню", callback_data="menu")
    else:
...
@router.callback_query(F.data.startswith("module_flowers_disable:"))
async def module_flowers_disable(callback: CallbackQuery) -> None:
    ms_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        ms = await session.scalar(
            select(ModuleSpeaker)
            .join(Module)
            .where(ModuleSpeaker.id == ms_id, Module.owner_user_id.in_(owner_ids))
            .options(selectinload(ModuleSpeaker.speaker), selectinload(ModuleSpeaker.module))
        )
        if not ms:
            await callback.answer("Спикер не найден или нет доступа.", show_alert=True)
            return
        old = {
            "speaker": ms.speaker.full_name,
            "flower_required": ms.flower_required,
            "flower_excluded": ms.flower_excluded,
            "setup": ms.setup or [],
        }
        ms.flower_required = False
        ms.flower_excluded = True
        ms.setup = [item for item in (ms.setup or []) if str(item).strip().lower() != "цветы"]
        reminders = (
            await session.scalars(
                select(Reminder).where(
                    Reminder.module_id == ms.module_id,
                    Reminder.speaker_id == ms.speaker_id,
                    Reminder.reminder_type == "flowers",
        ...
        await log_action(
            ...
            ms.id,
            old=old,
            new={"flower_required": False, "flower_excluded": True, "setup": ms.setup or []},
        )
        module_id = ms.module_id
        await session.commit()
    kb = InlineKeyboardBuilder()
    kb.button(text="🌸 К цветам модуля", callback_data=f"module_flowers:{module_id}")
    kb.button(text="⬅️ К модулю", callback_data=f"module:{module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.answer("Готово: убрал из цветов.")
    await send_or_edit(callback, "✅ Спикер убран из списка цветов по этому модулю.", kb)
...
async def upload_schedule(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        "📎 Отправьте сетку текстом прямо сюда или загрузите .txt.\n\n"
        "Лучший формат: день, затем строки вида 9:30-11:00 - Название, ниже Тема/Спикер/Формат.\n"
        "PDF/XLSX тоже можно, но основной надежный вариант теперь текстовый.",
        reply_markup=kb_back(module_id).as_markup(),
    )
    await callback.answer()
...
@router.callback_query(F.data.in_({"program", "pretty_schedule"}))
async def program_menu(callback: CallbackQuery) -> None:
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        modules = (
            await session.scalars(
    ...
    kb = InlineKeyboardBuilder()
    for module in modules:
        kb.button(text=f"{module.name} | {module.dates_label or date_range_label(module)}", callback_data=f"program_module:{module.id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    text = "<b>Программа</b>\n\nВыберите модуль." if modules else "Сначала создайте модуль."
    await send_or_edit(callback, text, kb)


@router.callback_query(F.data.startswith("program_module:"))
async def program_module(callback: CallbackQuery) -> None:
    module_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        permissions = await access_permissions(session, callback.from_user.id, callback.from_user.username)
        ...
        images = (
            await session.scalars(
                ...
                .where(PrettyScheduleImage.module_id == module_id)
                .order_by(PrettyScheduleImage.schedule_date, PrettyScheduleImage.created_at)
            )
        ).all()
    kb = InlineKeyboardBuilder()
    for image in images:
        day_label = display_program_day_label(image)
        kb.button(text=day_label, callback_data=f"pretty_schedule_show:{image.id}")
        if permission_allows(permissions, f"pretty_schedule_delete_ask:{image.id}"):
            kb.button(text=f"🗑 Удалить {day_label}", callback_data=f"pretty_schedule_delete_ask:{image.id}")
    if permission_allows(permissions, f"pretty_schedule_upload:{module_id}"):
        kb.button(text="➕ Добавить программу", callback_data=f"pretty_schedule_upload:{module_id}")
    kb.button(text="⬅️ К выбору модуля", callback_data="program")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    text = f"<b>Программа: {html_escape(module.name)}</b>\n\nВыберите день." if images else f"<b>Программа: {html_escape(module.name)}</b>\n\nПока нет загруженных картинок."
    await send_or_edit(callback, text, kb)


@router.callback_query(F.data.startswith("pretty_schedule_delete_ask:"))
async def pretty_schedule_delete_ask(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if not image:
            ...
            return
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ Да, удалить", callback_data=f"pretty_schedule_delete:{image_id}")
    if image.module_id:
        kb.button(text="❌ Отмена", callback_data=f"program_module:{image.module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, f"Удалить программу за {html_escape(display_program_day_label(image))}?", kb)


@router.callback_query(F.data.startswith("pretty_schedule_delete:"))
async def pretty_schedule_delete(callback: CallbackQuery, bot: Bot) -> None:
    image_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        image = await session.get(PrettyScheduleImage, image_id)
        if not image:
            ...
            return
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        module = await session.get(Module, image.module_id) if image.module_id else None
        if image.owner_user_id not in owner_ids:
            await callback.answer("Нет доступа к удалению.", show_alert=True)
            return
        day_label = image.day_label
        module_id = image.module_id
        file_path = image.file_path
        await log_action(session, callback.from_user.id, "delete_program", "pretty_schedule", image.id, old={"day": day_label})
        await session.delete(image)
        await session.commit()
        if module:
            await notify_program_change(bot, module, callback.from_user, "удалена", day_label)
    try:
        path = Path(file_path)
        if path.exists() and UPLOAD_DIR.resolve() in path.resolve().parents:
            path.unlink()
    except Exception as exc:
        log.warning("program image file delete failed: %s", exc)
    kb = InlineKeyboardBuilder()
    if module_id:
        kb.button(text="⬅️ К программе модуля", callback_data=f"program_module:{module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "✅ Программа удалена.", kb)


@router.callback_query(F.data.startswith("pretty_schedule_upload"))
async def pretty_schedule_upload(callback: CallbackQuery, state: FSMContext) -> None:
    module_id = int(callback.data.split(":")[1]) if ":" in callback.data else None
    await state.update_data(module_id=module_id)
    ...
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.edit_text("Введите день программы. Например: 23 июля или 23.07.2026", reply_markup=kb.as_markup())
    await callback.answer()
...
async def pretty_schedule_day_save(message: Message, state: FSMContext) -> None:
    ...
    if not label:
        await message.answer("Напишите день текстом, например: 23 июля.", reply_markup=kb_error_back().as_markup())
        return
    ...
    if data.get("module_id"):
        kb.button(text="⬅️ К программе модуля", callback_data=f"program_module:{data['module_id']}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await message.answer("Теперь отправьте картинку программы для этого дня.", reply_markup=kb.as_markup())
...
async def pretty_schedule_image_save(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    if message.photo:
        ...
        if not re.search(r"\.(jpg|jpeg|png|webp)$", src_name, flags=re.I):
            await message.answer("Нужна картинка: jpg, png или webp.", reply_markup=kb_error_back().as_markup())
            return
        file = await bot.get_file(doc.file_id)
    else:
        await message.answer("Отправьте картинку.", reply_markup=kb_error_back().as_markup())
        return
    ...
    async with SessionLocal() as session:
        ...
        if module_id:
            owner_ids = await visible_owner_ids(session, message.from_user.id, message.from_user.username)
            module = await session.get(Module, int(module_id))
            if not module or module.owner_user_id not in owner_ids:
                await state.clear()
                await message.answer("Модуль не найден или доступа нет.", reply_markup=kb_main().as_markup())
                return
            owner_id = module.owner_user_id
        ...
        await session.commit()
        if module:
            await notify_program_change(bot, module, message.from_user, "добавлена", data["day_label"])
    await state.clear()
    permissions = await effective_permissions(message.from_user.id, message.from_user.username)
    await message.answer("✅ Картинка программы сохранена.", reply_markup=kb_main(permissions).as_markup())
...
async def pretty_schedule_show(callback: CallbackQuery) -> None:
    ...
    kb = InlineKeyboardBuilder()
    if image.module_id:
        kb.button(text="⬅️ К программе модуля", callback_data=f"program_module:{image.module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.answer_photo(
        FSInputFile(path),
        caption=html_escape(display_program_day_label(image)),
...
async def receive_schedule_file(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        async with SessionLocal() as session:
            ...
            strict_text = is_strict_schedule_text(extracted)
            if is_structured_schedule_text(extracted):
                parsed = parse_structured_schedule_text(module, extracted)
                if settings.ai_enabled and openai_client and OPENAI_API_KEY and not strict_text:
                    parsed = await refine_structured_schedule_with_ai(module, parsed, require_ai=False)
            elif "CELL_BLOCKS:" in extracted:
...
@router.message(ScheduleUpload.waiting_file, F.text)
async def receive_schedule_text(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    module_id = data["module_id"]
    raw_text = message.text or ""
    await message.answer("Текст сетки получен. Разбираю дни, слоты, темы и спикеров.")
    try:
        async with SessionLocal() as session:
            module = await session.get(Module, module_id)
            settings = await get_settings(session, message.from_user.id)
            strict_text = is_strict_schedule_text(raw_text)
            parsed = parse_structured_schedule_text(module, raw_text)
            if settings.ai_enabled and openai_client and OPENAI_API_KEY and not strict_text:
                parsed = await refine_structured_schedule_with_ai(module, parsed, require_ai=False)
            validate_schedule_coverage(parsed, raw_text)
            validate_schedule_quality(parsed)
            upload = UploadedFile(
                module_id=module_id,
                file_name="telegram_text_schedule.txt",
                file_type="txt",
                file_path="telegram_text_message",
                parsed_json=parsed,
            )
            session.add(upload)
            await session.flush()
        ...
        await message.answer(
            "❌ Расписание не сохранено.\n\n"
            f"{html_escape(exc)}\n\n"
            "Проверьте текст: нужны даты дней и строки вида 9:30-11:00 - Название.",
            reply_markup=kb_back(module_id).as_markup(),
        )
        return
    kb = InlineKeyboardBuilder()
...
STRICT_SLOT_RE = re.compile(
    r"^\s*(\d{1,2}[:.]\d{2})\s*[-–—]\s*(\d{1,2}[:.]\d{2})\s*(?:[-–—]\s*)?(.+?)\s*$"
)


def normalize_strict_line(value: str) -> str:
    return re.sub(r"\s+", " ", (value or "").replace("—", "-").replace("–", "-")).strip()


def is_strict_schedule_text(source_text: str) -> bool:
    text = source_text or ""
    slot_count = sum(1 for line in text.splitlines() if STRICT_SLOT_RE.match(normalize_strict_line(line)))
    day_count = 0
    for raw_line in text.splitlines():
        line = normalize_strict_line(raw_line)
        if not line or STRICT_SLOT_RE.match(line):
            continue
        parsed_date = parse_date_value(line)
        if parsed_date and not re.search(r"\d{1,2}[:.]\d{2}\s*[-–—]\s*\d{1,2}[:.]\d{2}", line):
            day_count += 1
    return slot_count >= 2 and day_count >= 1


def split_strict_speaker_names(value: Any) -> list[str]:
    if not value:
        return []
    if isinstance(value, list):
        result: list[str] = []
        for item in value:
            result.extend(split_strict_speaker_names(item))
        return unique_names(result)

    raw = normalize_strict_line(str(value)).strip(" .")
    if not raw:
        return []
    raw = re.sub(r"(?i)^\s*(спикер(?:ы)?|ведущ(?:ий|ие)|модератор(?:ы)?)\s*:\s*", "", raw).strip()
    low_raw = raw.lower()
    if low_raw in {"уточнить", "нет", "не указан", "не указано", "спикера нет", "-"}:
        return []
    ...
    for chunk in chunks:
        for part in re.split(r"\s*,\s*", chunk):
            ...
            if not part:
                continue
            low = part.lower()
            if low in {"уточнить", "нет", "не указан", "не указано", "-"}:
                continue
            if part[:1].islower():
                continue
            ...
            words = words[:3]
            candidate = " ".join(words)
            cleaned = clean_speaker_name(candidate)
            if cleaned and (looks_like_speaker_name(cleaned) or (len(cleaned.split()) == 1 and cleaned[:1].isupper())):
                names.append(cleaned)
    return unique_names(names)


def parse_strict_schedule_text(module: Module, source_text: str) -> dict[str, Any] | None:
    if not is_strict_schedule_text(source_text):
        return None
    ...
    def ensure_day(day_date: date) -> dict[str, Any]:
        nonlocal current_day, current_item, current_date, day_theme, last_field
        current_date = day_date
        day_iso = day_date.isoformat()
        for day in days:
            if day.get("date") == day_iso:
                current_day = day
                current_item = None
                last_field = ""
                return day
        current_day = {"date": day_iso, "items": []}
    ...
    for raw_line in source_text.replace("\r\n", "\n").replace("\r", "\n").split("\n"):
        ...
        slot_match = STRICT_SLOT_RE.match(line)
        if slot_match:
            if current_day is None:
                ensure_day(module.start_date or datetime.now().date())
            start, end, title = slot_match.groups()
            ...
            current_item = {
                ...
                "format": "",
                "setup": [],
                "comment": f"day_theme={day_theme}" if day_theme else "",
            }
            current_day["items"].append(current_item)
            last_field = ""
            continue

        parsed_date = parse_date_value(line)
        if parsed_date and not re.search(r"\d{1,2}[:.]\d{2}\s*[-–—]\s*\d{1,2}[:.]\d{2}", line):
            ensure_day(parsed_date)
            continue
        ...
        if ":" in line:
            ...
            if low_key == "день":
                day_theme = value
                if current_day and not current_item:
                    current_day["theme"] = value
                last_field = "day"
                continue
            if current_item is None:
                continue
            if low_key in {"спикер", "спикеры", "ведущий", "ведущие", "модератор", "модераторы"}:
                speakers = split_strict_speaker_names(value)
                current_item["speaker_names"] = unique_names((current_item.get("speaker_names") or []) + speakers)
                current_item["speaker_name"] = "; ".join(current_item["speaker_names"])
                comment_tail = value
                for speaker in speakers:
                    comment_tail = re.sub(re.escape(speaker), "", comment_tail, flags=re.I).strip(" ,;.-")
                if comment_tail and comment_tail.lower() not in {"уточнить", "нет", "не указан", "не указано"}:
                    append_item_comment(current_item, f"{key}: {comment_tail}")
                last_field = "speaker"
                continue
            if low_key == "тема":
                if value:
                    append_item_comment(current_item, f"Тема: {value}")
                last_field = "comment"
                continue
            if low_key == "формат":
                current_item["format"] = value
                last_field = "format"
                continue
            if low_key in {"сетап", "setup"}:
                current_item["setup"] = split_setup(value)
                last_field = "setup"
                continue
            if low_key in {"локация", "место", "зал"}:
                current_item["location"] = value
                last_field = "location"
                continue
            if low_key in {"примечание", "комментарий", "коммент"}:
                append_item_comment(current_item, f"{key}: {value}")
                last_field = "comment"
                continue

        if current_item is not None:
            if last_field == "speaker":
                append_item_comment(current_item, line)
            elif last_field == "format":
                current_item["format"] = f"{current_item.get('format', '')} {line}".strip()
            else:
                append_item_comment(current_item, line)
    ...
    if not days:
        return None
    for day in days:
        day["items"] = sorted(day.get("items", []), key=lambda item: (time_to_minutes(item.get("start_time")), time_to_minutes(item.get("end_time"))))
    return {"module_name": module.name, "days": days}


def parse_structured_schedule_text(module: Module, source_text: str) -> dict[str, Any]:
    strict = parse_strict_schedule_text(module, source_text)
    if strict:
        return strict
    ...
    def ensure_day(day_date: date, label: str = "") -> dict[str, Any]:
        nonlocal current_day, current_item, day_theme
        day_iso = day_date.isoformat()
        for day in days:
            if day["date"] == day_iso:
                current_day = day
    ...
    for raw_line in text.split("\n"):
        ...
        slot_match = re.match(r"^(\d{1,2}[:.]\d{2})\s*-\s*(\d{1,2}[:.]\d{2})\s*-\s*(.+)$", line)
        if slot_match:
            detail_mode = False
            if current_day is None:
                fallback = module.start_date or datetime.now().date()
                ensure_day(fallback)
            start, end, title = slot_match.groups()
            title = title.strip(" .")
            speakers_from_title = infer_speakers_from_title(title)
            current_item = {
                "start_time": start.replace(".", ":"),
                "end_time": end.replace(".", ":"),
                "title": title,
                "speaker_name": "; ".join(speakers_from_title),
                "speaker_names": speakers_from_title,
                "location": module.location or "",
                "format": "",
                "comment": f"day_theme={day_theme}" if day_theme else "",
            }
        ...
        parsed_date = parse_date_value(line)
        if parsed_date and not re.search(r"\d{1,2}[:.]\d{2}\s*-", line):
            ensure_day(parsed_date, line)
            if " - " in line:
                day_theme = line.split(" - ", 1)[1].strip(" .")
            continue

        if line.lower().startswith("день:"):
            day_theme = line.split(":", 1)[1].strip(" .")
            continue
        ...
        if line.lower().startswith("тема / спикер / описание"):
            detail_mode = True
            continue

        if detail_mode and line.startswith("-"):
            detail = line.lstrip("-").strip()
            if detail:
                apply_freeform_detail(current_item, detail)
            continue
        ...
        if ":" in line:
            ...
            low_key = key.lower()
            value = value.strip(" .")
            if low_key in {"спикер", "спикеры", "ведущий", "ведущие"}:
                speakers, speaker_comment = parse_explicit_speakers(value)
                current_item["speaker_names"] = unique_names((current_item.get("speaker_names") or []) + speakers)
                current_item["speaker_name"] = "; ".join(current_item["speaker_names"])
                if speaker_comment:
                    append_item_comment(current_item, f"{key}: {speaker_comment}")
                continue
            if low_key == "тема":
                append_item_comment(current_item, f"Тема: {value}")
                if value and value.lower() not in current_item["title"].lower():
                    current_item["title"] = f"{current_item['title']} - {value}".strip(" -")
                continue
            if low_key == "формат":
                current_item["format"] = value
                continue
    ...
    if not days:
        raise RuntimeError("Не нашел дни в текстовой сетке. Начните день строкой вроде: 23 июля, четверг.")
    for day in days:
        day["items"] = sorted(day["items"], key=lambda item: (time_to_minutes(item["start_time"]), time_to_minutes(item["end_time"])))
    return {"module_name": module.name, "days": days}


def is_structured_schedule_text(source_text: str) -> bool:
    text = source_text or ""
    has_slots = len(re.findall(r"(?m)^\s*\d{1,2}[:.]\d{2}\s*[-–—]\s*\d{1,2}[:.]\d{2}\s*[-–—]", text)) >= 2
    has_dates = len(re.findall(r"(?im)^\s*\d{1,2}\s+[а-яё]+(?:,|\s|$)|^\s*\d{1,2}\.\d{1,2}", text)) >= 1
    return has_slots and has_dates


def infer_speakers_from_title(title: str) -> list[str]:
    title = title or ""
    candidates = []
    exact = clean_speaker_name(title)
    if looks_like_speaker_name(exact):
        candidates.append(exact)
    for match in re.finditer(r"\b(?:с|со)\s+([А-ЯЁ][а-яё]+(?:-[А-ЯЁ][а-яё]+)?\s+[А-ЯЁ][а-яё]+(?:-[А-ЯЁ][а-яё]+)?)", title):
        candidate = clean_speaker_name(match.group(1))
        if candidate:
            candidates.append(candidate)
    return unique_names(candidates)
...
def append_item_comment(item: dict[str, Any], text: str) -> None:
    text = (text or "").strip()
    if not text:
        return
    current = item.get("comment", "")
    item["comment"] = f"{current}; {text}".strip("; ")


def apply_freeform_detail(item: dict[str, Any], detail: str) -> None:
    low = detail.lower().strip()
    if not detail or low in {"?", "?кто", "кто", "уточнить"}:
        append_item_comment(item, f"Спикер: {detail}")
        return
    speakers, speaker_comment = parse_explicit_speakers(detail)
    title_low = (item.get("title") or "").lower()
    looks_like_description = any(marker in low for marker in (
        "работа ", "работа с", "станц", "стрельба", "урок", "квест", "лазертаг",
        "погружение", "предприниматель", "эксперт", "писатель", "победитель",
        "основатель", "полковник", "описание", "тема",
    ))
    if speakers and not (looks_like_description and not any(name.lower() in title_low for name in speakers)):
        item["speaker_names"] = unique_names((item.get("speaker_names") or []) + speakers)
        item["speaker_name"] = "; ".join(item["speaker_names"])
        if speaker_comment:
            append_item_comment(item, speaker_comment)
    else:
        append_item_comment(item, detail)


def parse_explicit_speakers(value: str) -> tuple[list[str], str]:
    raw = (value or "").strip()
    if not raw or raw.lower() in {"уточнить", "нет", "не указан", "не указано", "-"}:
        return [], raw
    comment_parts = []
    names = []
    for part in re.split(r"\s*;\s*|\s+ и \s+|\s*/\s*", raw):
        part = part.strip(" .")
        if not part:
            continue
        name_part = re.split(r"\s*,\s*|\s+-\s+", part, maxsplit=1)[0].strip()
        if name_part.lower() in {"уточнить", "нет", "не указан", "не указано"}:
            comment_parts.append(part)
            continue
        cleaned = clean_speaker_name(name_part)
        if cleaned and (looks_like_speaker_name(cleaned) or len(cleaned.split()) == 1):
            names.append(cleaned)
            if part != name_part:
                comment_parts.append(part)
        else:
            comment_parts.append(part)
    return unique_names(names), "; ".join(comment_parts)


async def refine_structured_schedule_with_ai(module: Module, parsed: dict[str, Any], require_ai: bool = False) -> dict[str, Any]:
    if not openai_client or not OPENAI_API_KEY:
        if require_ai:
            raise RuntimeError("AI API key is not configured")
        return parsed
    compact = json.dumps(parsed, ensure_ascii=False)
    if len(compact) > 25000:
        return parsed
    try:
        refined = await ai_json_create(
            (
                "Ты проверяешь уже структурированное расписание. Не меняй даты, start_time, end_time и количество слотов. "
                "Можно только аккуратно поправить title, speaker_names, speaker_name, format, location, comment. "
                "Спикеры - только люди. 'Жога', 'Баранников' можно оставить как спикеров, если они явно указаны в поле Спикер/Спикеры/Ведущий. "
                "Если после имени идет описание через запятую, описание перенеси в comment. "
                "Женские имена не помечай отдельно, просто оставь имя в speaker_names. Верни JSON по схеме."
            ),
            f"Модуль: {module.name}\nРасписание JSON:\n{compact}",
            schedule_json_schema(),
            "refined_structured_schedule",
        )
        if fixed_schedule_keys(refined) == fixed_schedule_keys(parsed):
            return refined
    except Exception as exc:
        log.warning("Structured schedule AI refine failed: %s", exc)
        if require_ai:
            raise
    return parsed
...
def schedule_preview(parsed: dict[str, Any]) -> str:
    ...
    for day in parsed.get("days", []):
        ...
        for item in day.get("items", []):
            ...
            if speakers:
                lines.append(f"{time_text} | <b>{html_escape(', '.join(speakers))}</b>")
            else:
...
async def schedule_confirm(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        await session.commit()
        permissions = await access_permissions(session, callback.from_user.id, callback.from_user.username)
        text, kb = await module_card(session, upload.module_id, permissions)
...
async def save_schedule_json(session: AsyncSession, module_id: int, parsed: dict[str, Any], user_id: int) -> None:
    ...
    for day in parsed.get("days", []):
        ...
        for item in day.get("items", []):
            ...
            setup = split_setup(item.get("setup") or item.get("setup_items") or [])
            if speakers:
...
async def manual_date(message: Message, state: FSMContext) -> None:
    ...
    if not parsed:
        await message.answer("Не понял дату. Попробуйте формат 2026-06-24.", reply_markup=kb_error_back().as_markup())
        return
...
async def manual_start(message: Message, state: FSMContext) -> None:
    if not parse_time(message.text):
        await message.answer("Не понял время. Например: 14:30", reply_markup=kb_error_back().as_markup())
        return
...
async def manual_end(message: Message, state: FSMContext) -> None:
    if not parse_time(message.text):
        await message.answer("Не понял время. Например: 15:20", reply_markup=kb_error_back().as_markup())
        return
...
async def schedule_now(callback: CallbackQuery) -> None:
    module_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        settings = await get_settings(session, callback.from_user.id)
        tz = ZoneInfo(settings.timezone or DEFAULT_TZ)
        now_local = datetime.now(tz)
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        stmt = (
            select(ScheduleItem)
            .join(Module)
            .where(
                Module.owner_user_id.in_(owner_ids),
                ScheduleItem.date == now_local.date(),
                ScheduleItem.start_time <= now_local.time(),
                ((ScheduleItem.end_time.is_(None)) | (ScheduleItem.end_time > now_local.time())),
            )
            .options(selectinload(ScheduleItem.speaker), selectinload(ScheduleItem.module))
            .order_by(ScheduleItem.start_time, ScheduleItem.end_time)
        )
        if module_id:
            stmt = stmt.where(ScheduleItem.module_id == module_id)
        ...
        lines = [f"▶️ <b>Что идет сейчас</b>", f"Время: {now_local:%d.%m.%Y %H:%M}", ""]
        if not grouped:
            lines += [
                "Сейчас по расписанию ничего не идет.",
                "",
                "Можно спокойно выдохнуть, проверить площадку или заглянуть в следующий слот.",
            ]
        else:
            for group_items in grouped.values():
                ...
                speaker_names = [row.speaker.full_name for row in group_items if row.speaker]
                setup_items = sorted(
                    {setup for row in group_items for setup in (row.setup or [])}
                    | {setup for ms in module_speakers for setup in (ms.setup or [])}
                )
                lines += [
                    f"<b>{time_label(item.start_time)}-{time_label(item.end_time)}</b>",
                    f"Модуль: {html_escape(item.module.name if item.module else '')}",
    ...
    kb = InlineKeyboardBuilder()
    if module_id:
        kb.button(text="📅 Расписание по дням", callback_data=f"schedule_days:{module_id}")
        kb.button(text="⬅️ К модулю", callback_data=f"module:{module_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "\n".join(lines), kb)
...
async def schedule_day(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        for _, group_items in grouped.items():
            ...
            needs_flowers = any(module_speaker_needs_flowers(ms) for ms in module_speakers)
            lines += [
                f"<b>{time_label(item.start_time)}-{time_label(item.end_time)}</b>",
            ...
            if speaker_names:
                lines.insert(
                    -1,
                    "🎤 <b>Спикеры:</b> " + ", ".join(f"<b>{html_escape(name)}</b>" for name in speaker_names),
                )
            if setup_items:
                lines.append(f"Сетап: {html_escape(', '.join(setup_items))}")
            if total_amount:
                lines.append(f"Сумма: {money(total_amount)}")
            if needs_flowers:
                lines.append("🌸 Нужны цветы")
            lines.append("")
            button_title = item.title[:32] + ("..." if len(item.title) > 32 else "")
            kb.button(
                text=f"{time_label(item.start_time)}-{time_label(item.end_time)} | {button_title or 'слот'}",
                callback_data=f"schedule_slot:{item.id}",
            )
        if module_id:
            kb.button(text="⬅️ К дням", callback_data=f"schedule_days:{module_id}")
        kb.button(text="🏠 Главное меню", callback_data="menu")
        kb.adjust(1)
    await send_or_edit(callback, "\n".join(lines), kb)


@router.callback_query(F.data.startswith("schedule_slot:"))
async def schedule_slot_open(callback: CallbackQuery) -> None:
    item_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        permissions = await access_permissions(session, callback.from_user.id, callback.from_user.username)
        item, group_items = await get_schedule_slot_group(session, item_id, owner_ids)
        if not item:
            await send_or_edit(callback, "Слот не найден или доступа нет.", kb_main(permissions))
            return
        text, kb = await schedule_slot_card(session, item, group_items, permissions)
    await send_or_edit(callback, text, kb)


@router.callback_query(F.data.startswith("schedule_slot_edit_title:"))
async def schedule_slot_edit_title(callback: CallbackQuery, state: FSMContext) -> None:
    item_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        item, _group_items = await get_schedule_slot_group(session, item_id, owner_ids)
        if not item:
            await callback.answer("Слот не найден или доступа нет.", show_alert=True)
            return
    await state.update_data(item_id=item_id)
    await state.set_state(ScheduleSlotEdit.title)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад к слоту", callback_data=f"schedule_slot:{item_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.edit_text("Введите новое название / тему слота:", reply_markup=kb.as_markup())
    await callback.answer()


@router.message(ScheduleSlotEdit.title)
async def schedule_slot_title_save(message: Message, state: FSMContext) -> None:
    title = message.text.strip()
    if not title:
        await message.answer("Название не должно быть пустым.", reply_markup=kb_error_back().as_markup())
        return
    data = await state.get_data()
    ...
    async with SessionLocal() as session:
        ...
        if not item:
            await state.clear()
            await message.answer("Слот не найден или доступа нет.", reply_markup=kb_main(permissions).as_markup())
            return
        old = {"title": item.title, "items": [row.id for row in group_items]}
        for row in group_items:
            row.title = title
        await log_action(session, message.from_user.id, "update_schedule_slot_title", "schedule_item", item.id, old=old, new={"title": title})
        await session.commit()
        item, group_items = await get_schedule_slot_group(session, item.id, owner_ids)
        text, kb = await schedule_slot_card(session, item, group_items, permissions)
    await state.clear()
    await message.answer("✅ Название слота обновлено.\n\n" + text, reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("schedule_slot_edit_time:"))
async def schedule_slot_edit_time(callback: CallbackQuery, state: FSMContext) -> None:
    item_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        item, _group_items = await get_schedule_slot_group(session, item_id, owner_ids)
        if not item:
            ...
            return
    await state.update_data(item_id=item_id)
    await state.set_state(ScheduleSlotEdit.time)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад к слоту", callback_data=f"schedule_slot:{item_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.edit_text(
        "Введите новое время слота.\n\nНапример: 09:45-10:30",
        reply_markup=kb.as_markup(),
    )
    await callback.answer()


@router.message(ScheduleSlotEdit.time)
async def schedule_slot_time_save(message: Message, state: FSMContext) -> None:
    raw = message.text.strip()
    parts = re.split(r"\s*[-–—]\s*", raw, maxsplit=1)
    if len(parts) != 2:
        await message.answer("Не понял время. Введите в формате 09:45-10:30.", reply_markup=kb_error_back().as_markup())
        return
    ...
    if time_to_minutes(end_time) <= time_to_minutes(start_time):
        await message.answer("Время окончания должно быть позже времени начала.", reply_markup=kb_error_back().as_markup())
        return
    data = await state.get_data()
    item_id = int(data["item_id"])
    async with SessionLocal() as session:
        ...
        if not item:
            ...
            return
        old = {
            "start_time": time_label(item.start_time),
            "end_time": time_label(item.end_time),
            "items": [row.id for row in group_items],
        }
        for row in group_items:
            row.start_time = start_time
            row.end_time = end_time
        await recreate_reminders_for_module(session, item.module_id, message.from_user.id)
        await log_action(
            session,
            message.from_user.id,
            "update_schedule_slot_time",
            "schedule_item",
            item.id,
            old=old,
            new={"start_time": time_label(start_time), "end_time": time_label(end_time)},
        )
        await session.commit()
        item, group_items = await get_schedule_slot_group(session, item.id, owner_ids)
        text, kb = await schedule_slot_card(session, item, group_items, permissions)
    await state.clear()
    await message.answer("✅ Время слота обновлено.\n\n" + text, reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("schedule_slot_edit_setup:"))
async def schedule_slot_edit_setup(callback: CallbackQuery, state: FSMContext) -> None:
    item_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        item, _group_items = await get_schedule_slot_group(session, item_id, owner_ids)
        if not item:
            ...
            return
    await state.update_data(item_id=item_id)
    await state.set_state(ScheduleSlotEdit.setup)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад к слоту", callback_data=f"schedule_slot:{item_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.edit_text(
        "Введите сетап слота через запятую.\n\nНапример: микрофон, презентация, кликер, вода\n\nЧтобы очистить сетап, отправьте -",
        reply_markup=kb.as_markup(),
    )
    await callback.answer()


@router.message(ScheduleSlotEdit.setup)
async def schedule_slot_setup_save(message: Message, state: FSMContext) -> None:
    raw = message.text.strip()
    setup = [] if raw == "-" else split_setup(raw)
    data = await state.get_data()
    item_id = int(data["item_id"])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, message.from_user.id, message.from_user.username)
        ...
        if not item:
            ...
            await message.answer("Слот не найден или доступа нет.", reply_markup=kb_main(permissions).as_markup())
            return
        old = {"setup": item.setup or [], "items": [row.id for row in group_items]}
        for row in group_items:
            row.setup = setup
        await log_action(session, message.from_user.id, "update_schedule_slot_setup", "schedule_item", item.id, old=old, new={"setup": setup})
        await session.commit()
        item, group_items = await get_schedule_slot_group(session, item.id, owner_ids)
        text, kb = await schedule_slot_card(session, item, group_items, permissions)
    await state.clear()
    await message.answer("✅ Сетап слота обновлен.\n\n" + text, reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("schedule_slot_edit_speakers:"))
async def schedule_slot_edit_speakers(callback: CallbackQuery, state: FSMContext) -> None:
    item_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
        item, _group_items = await get_schedule_slot_group(session, item_id, owner_ids)
        if not item:
            ...
            return
    await state.update_data(item_id=item_id)
    await state.set_state(ScheduleSlotEdit.speakers)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад к слоту", callback_data=f"schedule_slot:{item_id}")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.edit_text(
        "Введите спикеров слота через запятую.\n\nНапример: Иван Иванов, Мария Петрова\n\nЧтобы убрать спикеров из слота, отправьте -",
        reply_markup=kb.as_markup(),
    )
    await callback.answer()


@router.message(ScheduleSlotEdit.speakers)
async def schedule_slot_speakers_save(message: Message, state: FSMContext) -> None:
    raw = message.text.strip()
    speaker_names = parse_slot_speaker_input(raw)
    data = await state.get_data()
    item_id = int(data["item_id"])
    async with SessionLocal() as session:
        owner_ids = await visible_owner_ids(session, message.from_user.id, message.from_user.username)
        ...
        module = item.module
        old = {
            "speaker_ids": [row.speaker_id for row in group_items],
            "speaker_names": [row.speaker.full_name for row in group_items if row.speaker],
            "items": [row.id for row in group_items],
        }
        speaker_targets: list[tuple[int | None, Speaker | None]] = []
        for speaker_name in speaker_names:
            speaker = await find_speaker(session, module.owner_user_id, speaker_name)
            if not speaker:
                speaker = Speaker(
                    full_name=speaker_name,
                    gender=infer_gender_from_name(speaker_name),
                ...
                session.add(speaker)
                await session.flush()
            ms = await ensure_module_speaker(session, module, speaker, topic=item.title)
            if speaker.gender.lower().startswith("жен"):
                ms.flower_required = True
            speaker_targets.append((speaker.id, speaker))
        if not speaker_targets:
            speaker_targets = [(None, None)]

        base_rows = sorted(group_items, key=lambda row: row.id)
        for index, (speaker_id, speaker) in enumerate(speaker_targets):
            if index < len(base_rows):
                base_rows[index].speaker_id = speaker_id
                base_rows[index].speaker = speaker
            else:
                session.add(
                    ScheduleItem(
        ...
        for row in base_rows[len(speaker_targets):]:
            reminders = (await session.scalars(select(Reminder).where(Reminder.schedule_item_id == row.id))).all()
            for reminder in reminders:
                await session.delete(reminder)
            await session.delete(row)
        await session.flush()
        new_item_id = base_rows[0].id if base_rows else item.id
        await recreate_reminders_for_module(session, module.id, message.from_user.id)
        await log_action(
            session,
            message.from_user.id,
            "update_schedule_slot_speakers",
            "schedule_item",
            new_item_id,
            old=old,
            new={"speaker_names": speaker_names},
        )
        await session.commit()
        item, group_items = await get_schedule_slot_group(session, new_item_id, owner_ids)
        text, kb = await schedule_slot_card(session, item, group_items, permissions)
    await state.clear()
    await message.answer("✅ Спикеры слота обновлены.\n\n" + text, reply_markup=kb.as_markup())


async def recreate_reminders_for_module(session: AsyncSession, module_id: int, user_id: int) -> None:
    module = await session.get(Module, module_id)
    owner_user_id = module.owner_user_id if module else user_id
    settings = await get_settings(session, owner_user_id)
    tz = ZoneInfo(settings.timezone)
    recipient_ids = await module_access_user_ids(session, owner_user_id)
    subscribed_recipient_ids = []
    for recipient_id in recipient_ids:
        recipient_settings = await get_settings(session, recipient_id)
        if recipient_settings.team_reminders_enabled:
            subscribed_recipient_ids.append(recipient_id)
    for old in (await session.scalars(select(Reminder).where(Reminder.module_id == module_id, Reminder.is_sent == False))).all():
        if old.reminder_type in {"performance", "documents", "flowers"}:
            await session.delete(old)
    items = (
        await session.scalars(
            select(ScheduleItem)
    ...
    for item in items:
        ...
        if settings.performance_reminders_enabled:
            remind_at = start_dt - timedelta(minutes=settings.performance_reminder_minutes)
            if remind_at > now_utc:
                for recipient_id in subscribed_recipient_ids:
                    session.add(Reminder(module_id=item.module_id, speaker_id=item.speaker_id, schedule_item_id=item.id, reminder_type="performance", remind_at=remind_at, target_user_id=recipient_id))
        if settings.docs_reminders_enabled and ms and ms.is_paid:
            remind_at = start_dt - timedelta(minutes=settings.docs_reminder_minutes)
            if remind_at > now_utc:
                session.add(Reminder(module_id=item.module_id, speaker_id=item.speaker_id, schedule_item_id=item.id, reminder_type="documents", remind_at=remind_at, target_user_id=owner_user_id))
        if settings.flowers_reminders_enabled and module_speaker_needs_flowers(ms) and item.end_time:
            remind_at = end_dt - timedelta(minutes=settings.flowers_reminder_minutes)
            if remind_at > now_utc:
                for recipient_id in subscribed_recipient_ids:
                    session.add(Reminder(module_id=item.module_id, speaker_id=item.speaker_id, schedule_item_id=item.id, reminder_type="flowers", remind_at=remind_at, target_user_id=recipient_id))


async def recreate_reminders_for_owner(session: AsyncSession, owner_user_id: int) -> None:
    module_ids = (
        await session.scalars(select(Module.id).where(Module.owner_user_id == owner_user_id))
    ).all()
    for module_id in module_ids:
        await recreate_reminders_for_module(session, module_id, owner_user_id)
...
async def send_due_reminders(bot: Bot) -> None:
    async with SessionLocal() as session:
        ...
        user_reminders = (
            await session.scalars(
                select(UserReminder)
                .where(UserReminder.is_sent == False, UserReminder.remind_at <= now)
        ...
        for rem in user_reminders:
            try:
                sender_name = html_escape(rem.sender_name or "пользователь")
                await bot.send_message(
                    rem.target_user_id,
                    "\n".join([
                        "⏰ <b>Напоминание</b>",
                        f"От: <a href=\"tg://user?id={rem.sender_user_id}\">{sender_name}</a>",
                        "",
                        html_escape(rem.text),
                    ]),
                )
                rem.is_sent = True
                rem.sent_at = datetime.utcnow()
            except Exception as exc:
                log.warning("cannot send user reminder %s: %s", rem.id, exc)
        await session.commit()


async def build_reminder_text(session: AsyncSession, rem: Reminder) -> str:
    module = await session.get(Module, rem.module_id)
    settings = await get_settings(session, module.owner_user_id if module else rem.target_user_id)
    ...
    if rem.reminder_type == "documents" and ms:
        ...
        lines = [
            ...
            f"⏰ Выступление через {settings.docs_reminder_minutes} минут",
            f"💵 Сумма: {money(ms.amount)}",
    ...
    if rem.reminder_type == "flowers":
        return "\n".join([
            f"🌸 <b>Через {settings.flowers_reminder_minutes} минут нужно вынести цветы</b>",
            "",
    ...
    lines = [
        f"🔔 <b>Через {settings.performance_reminder_minutes} минут выступает спикер</b>",
        "",
    ...
    if ms and ms.setup:
        lines.extend(["", "⚙️ Сетап:", *(f"- {html_escape(x)}" for x in ms.setup)])
    lines.extend([
        "",
...
async def reminder_recipients(session: AsyncSession, user_id: int, username: str | None, scope: str) -> list[int]:
    if scope == "self":
        return [user_id]
    owner_id = await owner_user_id_for(session, user_id, username)
    recipients = {owner_id}
    grants = (
        await session.scalars(
            select(AccessGrant).where(AccessGrant.owner_user_id == owner_id, AccessGrant.allowed_user_id.is_not(None))
        )
    ).all()
    recipients.update(grant.allowed_user_id for grant in grants if grant.allowed_user_id)
    return sorted(recipients)


async def owner_user_id_for(session: AsyncSession, user_id: int, username: str | None = None) -> int:
    if not ALLOWED_USER_IDS or user_id in ALLOWED_USER_IDS:
        return user_id
    uname = normalize_username(username)
    ...
    if grant:
        if grant.allowed_user_id is None and uname:
            grant.allowed_user_id = user_id
        return grant.owner_user_id
    return user_id


async def module_access_user_ids(session: AsyncSession, owner_id: int) -> list[int]:
    recipients = {owner_id}
    grants = (
        await session.scalars(
            select(AccessGrant).where(AccessGrant.owner_user_id == owner_id, AccessGrant.allowed_user_id.is_not(None))
...
async def broadcast_start(callback: CallbackQuery, state: FSMContext) -> None:
    await state.set_state(BroadcastCreate.content)
    kb = InlineKeyboardBuilder()
    kb.button(text="⬅️ Назад", callback_data="menu")
    await send_or_edit(
        callback,
        "📣 <b>Рассылка от имени бота</b>\n\nОтправьте одним сообщением текст, фото или документ. После этого бот попросит подтверждение.",
        kb,
    )


@router.message(BroadcastCreate.content)
async def broadcast_content(message: Message, state: FSMContext) -> None:
    ...
    kb.adjust(1)
    await message.answer(
        "Проверьте сообщение выше. Отправить его всем пользователям вашей группы доступа?",
        reply_markup=kb.as_markup(),
    )


@router.callback_query(F.data.in_({"broadcast_confirm", "broadcast_cancel"}))
async def broadcast_finish(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    if callback.data == "broadcast_cancel":
        await state.clear()
        permissions = await effective_permissions(callback.from_user.id, callback.from_user.username)
        await send_or_edit(callback, "Рассылка отменена.", kb_main(permissions))
        return
    data = await state.get_data()
    source_chat_id = data.get("source_chat_id")
    source_message_id = data.get("source_message_id")
    if not source_chat_id or not source_message_id:
        await callback.answer("Сообщение для рассылки не найдено. Начните заново.", show_alert=True)
        await state.clear()
        return
    async with SessionLocal() as session:
        owner_id = await owner_user_id_for(session, callback.from_user.id, callback.from_user.username)
        recipients = await module_access_user_ids(session, owner_id)
        await log_action(
            session,
            callback.from_user.id,
            "broadcast",
            "bot_users",
            None,
            new={"recipients": len(recipients), "source_message_id": source_message_id},
        )
        await session.commit()
    sent = 0
    failed = 0
    for recipient_id in recipients:
        try:
            ...
            failed += 1
            log.warning("broadcast failed for %s: %s", recipient_id, exc)
    await state.clear()
    permissions = await effective_permissions(callback.from_user.id, callback.from_user.username)
    text = f"✅ Рассылка завершена. Доставлено: {sent}."
    if failed:
        text += f" Не доставлено: {failed}."
    await send_or_edit(callback, text, kb_main(permissions))


async def send_photo_report_reminders(bot: Bot) -> None:
    now_utc = datetime.utcnow().replace(tzinfo=ZoneInfo("UTC"))
    async with SessionLocal() as session:
        settings_rows = (await session.scalars(select(AppSetting))).all()
        known_users = {settings.user_id: settings for settings in settings_rows}
        owners = {module.owner_user_id for module in (await session.scalars(select(Module))).all()}
        for owner_id in owners:
            settings = known_users.get(owner_id) or AppSetting(user_id=owner_id)
            local_now = now_utc.astimezone(ZoneInfo(settings.timezone))
            if not (9 <= local_now.hour <= 21):
                continue
            today = local_now.date()
            modules = (
                await session.scalars(
                    select(Module).where(
                        Module.owner_user_id == owner_id,
                        Module.start_date.is_not(None),
                        Module.end_date.is_not(None),
                        Module.start_date <= today,
                        Module.end_date >= today,
                    )
                )
            ).all()
            for module in modules:
                message_index = (local_now.hour + module.id) % len(PHOTO_REPORT_MESSAGES)
                text = "\n".join([
                    PHOTO_REPORT_MESSAGES[message_index],
                    "",
                    f"📁 Модуль: {html_escape(module.name)}",
                    "Не забываем делать фото для отчета.",
                ])
                for recipient_id in await module_access_user_ids(session, owner_id):
                    try:
                        await bot.send_message(recipient_id, text)
                    except Exception as exc:
                        log.warning("photo report reminder failed to %s: %s", recipient_id, exc)


@router.callback_query(F.data == "quick_reminder")
async def quick_reminder_start(callback: CallbackQuery, state: FSMContext) -> None:
    await state.set_state(QuickReminderCreate.text)
    kb = InlineKeyboardBuilder()
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "⏰ Напишите текст напоминания.", kb)


@router.message(QuickReminderCreate.text)
async def quick_reminder_text(message: Message, state: FSMContext) -> None:
    text = message.text.strip()
    if not text:
        await message.answer("Напишите текст напоминания.", reply_markup=kb_error_back().as_markup())
        return
    await state.update_data(text=text)
    await state.set_state(QuickReminderCreate.when)
    await message.answer("Когда напомнить? Например: 24.07 14:30 или 2026-07-24 14:30")


@router.message(QuickReminderCreate.when)
async def quick_reminder_when(message: Message, state: FSMContext) -> None:
    async with SessionLocal() as session:
        settings = await get_settings(session, message.from_user.id)
        timezone = settings.timezone or DEFAULT_TZ
    tz = ZoneInfo(timezone)
    when = parse_reminder_datetime(message.text, timezone)
    if not when:
        await message.answer("Не понял дату и время. Пример: 24.07 14:30", reply_markup=kb_error_back().as_markup())
        return
    selected_local = when.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz)
    min_local = reminder_min_local(tz)
    if selected_local < min_local:
        await message.answer(
            "\n".join([
                "⏰ <b>Это время уже прошло</b>",
                "",
                f"Выберите время не раньше <b>{min_local.strftime('%d.%m %H:%M')}</b>.",
                f"Например: <code>{min_local.strftime('%d.%m %H:%M')}</code>",
            ]),
            reply_markup=kb_error_back().as_markup(),
        )
        return
    await state.update_data(remind_at=when.isoformat())
    kb = InlineKeyboardBuilder()
    kb.button(text="👤 Только мне", callback_data="quick_reminder_send:self")
    kb.button(text="👥 Всем с доступом", callback_data="quick_reminder_send:all")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await message.answer("Кому отправить напоминание?", reply_markup=kb.as_markup())


@router.callback_query(F.data.startswith("quick_reminder_send:"))
async def quick_reminder_send(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    scope = callback.data.split(":")[1]
    data = await state.get_data()
    reminder_text = data.get("text", "")
    remind_at = datetime.fromisoformat(data["remind_at"]) if data.get("remind_at") else None
    if not reminder_text:
        await callback.answer("Текст напоминания потерялся. Создайте заново.", show_alert=True)
        await state.clear()
        return
    if not remind_at or remind_at <= datetime.utcnow():
        await callback.answer("Время напоминания уже прошло. Создайте напоминание заново.", show_alert=True)
        await state.clear()
        return
    sender_name = callback.from_user.full_name or callback.from_user.username or str(callback.from_user.id)
    async with SessionLocal() as session:
        owner_id = await owner_user_id_for(session, callback.from_user.id, callback.from_user.username)
        recipients = await reminder_recipients(session, callback.from_user.id, callback.from_user.username, scope)
        ...
        await log_action(
            ...
            "user_reminder",
            None,
            new={"batch_id": batch_id, "scope": scope, "remind_at": remind_at.isoformat(), "recipients": len(recipients)},
        )
        owner_settings = await get_settings(session, owner_id)
        await session.commit()
    owner_tz = ZoneInfo(owner_settings.timezone or DEFAULT_TZ)
    local_when = remind_at.replace(tzinfo=ZoneInfo("UTC")).astimezone(owner_tz)
    owner_notice = "\n".join([
        "⏰ <b>Новая напоминалка</b>",
        "",
        f"Кто поставил: {actor_mention(callback.from_user)}",
        f"На время: <b>{local_when:%d.%m.%Y %H:%M}</b>",
        f"Кому: {'всем с доступом' if scope == 'all' else 'только автору'}",
        f"Получателей: {len(recipients)}",
        "",
        f"Текст: {html_escape(reminder_text)}",
    ])
    try:
        await bot.send_message(owner_id, owner_notice)
    except Exception as exc:
        log.warning("cannot notify reminder owner %s: %s", owner_id, exc)
    await state.clear()
    permissions = await effective_permissions(callback.from_user.id, callback.from_user.username)
    await send_or_edit(callback, f"✅ Напоминание запланировано. Получателей: {len(recipients)}.", kb_main(permissions))
...
async def reminders_view(callback: CallbackQuery) -> None:
    page = int(callback.data.rsplit(":", 1)[1]) if callback.data.startswith("reminders_page:") else 0
    async with SessionLocal() as session:
        settings = await get_settings(session, callback.from_user.id)
        automatic = (
            await session.scalars(
        ...
        is_owner = not ALLOWED_USER_IDS or callback.from_user.id in ALLOWED_USER_IDS
        if is_owner:
            group_user_ids = await module_access_user_ids(session, callback.from_user.id)
            personal_stmt = select(UserReminder).where(
                UserReminder.is_sent == False,
                (UserReminder.owner_user_id == callback.from_user.id)
                | (UserReminder.sender_user_id.in_(group_user_ids))
                | (UserReminder.target_user_id.in_(group_user_ids)),
            )
        else:
            personal_stmt = select(UserReminder).where(
                UserReminder.is_sent == False,
                (UserReminder.sender_user_id == callback.from_user.id)
                | (UserReminder.target_user_id == callback.from_user.id),
            )
        personal = (await session.scalars(personal_stmt.order_by(UserReminder.remind_at))).all()
        await session.commit()
    tz = ZoneInfo(settings.timezone or DEFAULT_TZ)
    labels = {"performance": "выступление", "documents": "документы", "flowers": "цветы"}
    lines = [
        "🔔 <b>Ближайшие напоминания</b>",
        "",
        f"Уведомления о спикерах и цветах: <b>{'включены' if settings.team_reminders_enabled else 'выключены'}</b>",
        "",
    ]
    if automatic and page == 0:
        lines.append("<b>Автоматические:</b>")
        for rem in automatic:
            local = rem.remind_at.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz)
            lines.append(f"• {local:%d.%m.%Y %H:%M} — {labels.get(rem.reminder_type, rem.reminder_type)}")
        lines.append("")
    grouped: dict[str, list[UserReminder]] = defaultdict(list)
    for rem in personal:
        grouped[rem.batch_id or f"id:{rem.id}"].append(rem)
    kb = InlineKeyboardBuilder()
    kb.button(
        text="🔕 Отключить уведомления" if settings.team_reminders_enabled else "🔔 Включить уведомления",
        callback_data="team_reminders_toggle",
    )
    grouped_rows = list(grouped.values())
    page_size = 8
    page_count = max(1, (len(grouped_rows) + page_size - 1) // page_size)
    page = min(max(page, 0), page_count - 1)
    visible_groups = grouped_rows[page * page_size : (page + 1) * page_size]
    if visible_groups:
        lines.append("<b>Напоминалки пользователей:</b>")
        if page_count > 1:
            lines.append(f"Страница {page + 1} из {page_count}")
            lines.append("")
        for index, rows in enumerate(visible_groups, page * page_size + 1):
            rem = rows[0]
            local = rem.remind_at.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz)
            sender = html_escape(rem.sender_name or str(rem.sender_user_id))
            reminder_preview = rem.text if len(rem.text) <= 240 else rem.text[:237].rstrip() + "..."
            lines += [
                f"<b>{index}. {local:%d.%m.%Y %H:%M}</b>",
                f"От: <a href=\"tg://user?id={rem.sender_user_id}\">{sender}</a>",
                f"Получателей: {len({row.target_user_id for row in rows})}",
                f"Текст: {html_escape(reminder_preview)}",
                "",
            ]
            if is_owner:
                kb.button(text=f"🗑 Удалить №{index}", callback_data=f"user_reminder_delete_ask:{rem.id}")
    if page_count > 1:
        if page > 0:
            kb.button(text="⬅️ Раньше", callback_data=f"reminders_page:{page - 1}")
        if page + 1 < page_count:
            kb.button(text="Позже ➡️", callback_data=f"reminders_page:{page + 1}")
    if not automatic and not grouped_rows:
        lines.append("Активных напоминаний пока нет.")
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, "\n".join(lines), kb)


@router.callback_query(F.data == "team_reminders_toggle")
async def team_reminders_toggle(callback: CallbackQuery) -> None:
    async with SessionLocal() as session:
        settings = await get_settings(session, callback.from_user.id)
        settings.team_reminders_enabled = not settings.team_reminders_enabled
        enabled = settings.team_reminders_enabled
        if enabled:
            owner_ids = await visible_owner_ids(session, callback.from_user.id, callback.from_user.username)
            module_ids = (
                await session.scalars(select(Module.id).where(Module.owner_user_id.in_(owner_ids)))
            ...
            for module_id in module_ids:
                await recreate_reminders_for_module(session, module_id, callback.from_user.id)
        else:
            rows = (
                await session.scalars(
                    select(Reminder).where(
                        Reminder.target_user_id == callback.from_user.id,
                        Reminder.is_sent == False,
                        Reminder.reminder_type.in_({"performance", "flowers"}),
                    )
                )
            ).all()
            for row in rows:
        ...
        await log_action(
            session,
            callback.from_user.id,
            "toggle_team_reminders",
            "settings",
            callback.from_user.id,
            new={"team_reminders_enabled": enabled},
        )
        await session.commit()
    await reminders_view(callback)


@router.callback_query(F.data.startswith("user_reminder_delete_ask:"))
async def user_reminder_delete_ask(callback: CallbackQuery) -> None:
    reminder_id = int(callback.data.rsplit(":", 1)[1])
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ Да, удалить", callback_data=f"user_reminder_delete:{reminder_id}")
    kb.button(text="❌ Отмена", callback_data="reminders")
    kb.adjust(1)
    await send_or_edit(
        callback,
        "🗑 <b>Удалить напоминалку?</b>\n\nОна будет удалена у всех получателей и больше не придет.",
        kb,
    )


@router.callback_query(F.data.startswith("user_reminder_delete:"))
async def user_reminder_delete(callback: CallbackQuery) -> None:
    if ALLOWED_USER_IDS and callback.from_user.id not in ALLOWED_USER_IDS:
        await callback.answer("Удалять напоминалки может только владелец.", show_alert=True)
        return
    reminder_id = int(callback.data.rsplit(":", 1)[1])
    async with SessionLocal() as session:
        rem = await session.get(UserReminder, reminder_id)
        if not rem:
            await callback.answer("Напоминалка уже удалена.", show_alert=True)
            return
        group_user_ids = await module_access_user_ids(session, callback.from_user.id)
        belongs_to_owner = (
            rem.owner_user_id == callback.from_user.id
            or rem.sender_user_id in group_user_ids
            or rem.target_user_id in group_user_ids
        )
        if not belongs_to_owner:
            await callback.answer("Эта напоминалка относится к другой группе доступа.", show_alert=True)
            return
        if rem.batch_id:
            rows = (
                await session.scalars(
                    select(UserReminder).where(UserReminder.batch_id == rem.batch_id, UserReminder.is_sent == False)
                )
            ).all()
        else:
            rows = [rem]
        ...
        await log_action(
            ...
            "user_reminder",
            reminder_id,
            old={"batch_id": rem.batch_id, "rows": len(rows), "text": rem.text[:500]},
        )
        await session.commit()
    await reminders_view(callback)
...
async def settings_view(callback: CallbackQuery) -> None:
    ...
    text = "\n".join([
        ...
        f"Напоминание перед выступлением: {'включено' if s.performance_reminders_enabled else 'выключено'} ({s.performance_reminder_minutes} мин.)",
        f"Напоминание о документах: {'включено' if s.docs_reminders_enabled else 'выключено'} ({s.docs_reminder_minutes} мин.)",
        f"Напоминание о цветах: {'включено' if s.flowers_reminders_enabled else 'выключено'} ({s.flowers_reminder_minutes} мин. до конца)",
    ...
    buttons = [
        ...
        (f"{'✅' if s.performance_reminders_enabled else '❌'} Выступления", "settings_toggle_reminder:performance"),
        (f"{'✅' if s.docs_reminders_enabled else '❌'} Документы", "settings_toggle_reminder:documents"),
        (f"{'✅' if s.flowers_reminders_enabled else '❌'} Цветы", "settings_toggle_reminder:flowers"),
    ...
    kb.adjust(1, 3, 2, 2, 2, 1, 1, 1, 1)
    await send_or_edit(callback, text, kb)


@router.callback_query(F.data == "bot_users")
async def bot_users(callback: CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        allowed_ids = {callback.from_user.id} | {grant.allowed_user_id for grant in grants if grant.allowed_user_id}
        allowed_usernames = {normalize_username(grant.username) for grant in grants if grant.username}
        rows = (
            await session.scalars(
                select(BotUserActivity).order_by(BotUserActivity.last_seen_at.desc())
            )
        ).all()
    visible_rows = [
        row for row in rows
        if row.user_id in allowed_ids or (row.username and normalize_username(row.username) in allowed_usernames)
    ]
    seen_usernames = {normalize_username(row.username) for row in visible_rows if row.username}
    lines = ["👥 <b>Пользователи бота</b>", ""]
    if visible_rows:
        for i, row in enumerate(visible_rows, 1):
            username = f"@{html_escape(row.username)}" if row.username else f"id:{row.user_id}"
            full_name = f" | {html_escape(row.full_name)}" if row.full_name else ""
            last_seen = row.last_seen_at.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz)
            lines += [
                f"<b>{i}. {username}</b>{full_name}",
                f"Последний раз: {last_seen:%d.%m.%Y %H:%M}",
                f"Действие: {html_escape(row.last_action or 'не указано')}",
                "",
            ]
    missing = [grant for grant in grants if normalize_username(grant.username) not in seen_usernames and not grant.allowed_user_id]
    if missing:
        lines.append("<b>Доступ выдан, но человек ещё не заходил:</b>")
        for grant in missing:
            lines.append(f"@{html_escape(grant.username)}")
        lines.append("")
    if not visible_rows and not missing:
        lines.append("Пока никто не проявлял активность. Тихо, как перед первым кофе-брейком.")
    kb = InlineKeyboardBuilder()
    ...
    text = "\n".join(lines)
    if len(text) > 3900:
        text = text[:3800] + "\n\n...список сокращен, остальные участники ниже по активности."
    await send_or_edit(callback, text, kb)
...
async def access_add_save(message: Message, state: FSMContext) -> None:
    ...
    if not username:
        await message.answer("Не понял username. Отправьте в формате @username.", reply_markup=kb_error_back().as_markup())
        return
    async with SessionLocal() as session:
...
async def settings_docs_toggle(callback: CallbackQuery) -> None:
    async with SessionLocal() as session:
        ...
        await recreate_reminders_for_owner(session, callback.from_user.id)
        await session.commit()
    await settings_view(callback)


@router.callback_query(F.data.startswith("settings_toggle_reminder:"))
async def settings_toggle_reminder(callback: CallbackQuery) -> None:
    reminder_type = callback.data.rsplit(":", 1)[1]
    fields = {
        "performance": "performance_reminders_enabled",
        "documents": "docs_reminders_enabled",
        "flowers": "flowers_reminders_enabled",
    }
    field = fields.get(reminder_type)
    if not field:
        await callback.answer("Неизвестный тип напоминания.", show_alert=True)
        return
    async with SessionLocal() as session:
        s = await get_settings(session, callback.from_user.id)
        ...
        await log_action(
            session,
            callback.from_user.id,
            "toggle_automatic_reminder",
            "settings",
            callback.from_user.id,
            new={field: getattr(s, field)},
        )
        await session.commit()
    await settings_view(callback)
...
async def settings_save(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if field == "timezone":
            ...
            field_map = {
                "performance_minutes": "performance_reminder_minutes",
                "flowers_minutes": "flowers_reminder_minutes",
                "post_interval": "post_module_interval_days",
            }
            setattr(s, field_map[field], max(1, int(re.sub(r"\D", "", value) or 1)))
        if field in {"timezone", "performance_minutes", "flowers_minutes"}:
            await recreate_reminders_for_owner(session, message.from_user.id)
        await session.commit()
    await state.clear()
...
async def natural_text(message: Message, state: FSMContext, bot: Bot) -> None:
    current = await state.get_state()
    if current:
        return
    if ALLOWED_USER_IDS and message.from_user.id not in ALLOWED_USER_IDS:
        async with SessionLocal() as session:
            owner_ids = [owner_id for owner_id in await visible_owner_ids(session, message.from_user.id, message.from_user.username) if owner_id != message.from_user.id]
            await session.commit()
        notify_ids = owner_ids or list(ALLOWED_USER_IDS)
        username = f"@{message.from_user.username}" if message.from_user.username else "username не указан"
        request_text = "\n".join([
            "🤖 <b>Свободный ИИ-запрос в боте</b>",
            "",
            f"Кто: {actor_mention(message.from_user)}",
            f"Username: {html_escape(username)}",
            f"ID: <code>{message.from_user.id}</code>",
        ...
        for owner_id in notify_ids:
            try:
                await bot.send_message(owner_id, request_text)
            except Exception as exc:
                log.warning("cannot notify owner %s about natural AI text: %s", owner_id, exc)
    async with SessionLocal() as session:
        settings = await get_settings(session, message.from_user.id)
        permissions = await access_permissions(session, message.from_user.id, message.from_user.username)
...
async def init_db() -> None:
    async with engine.begin() as conn:
        ...
        if dialect == "sqlite":
            ...
            rows = await conn.execute(sql_text("PRAGMA table_info(pretty_schedule_images)"))
            pretty_columns = {row[1] for row in rows.fetchall()}
            if pretty_columns and "module_id" not in pretty_columns:
                await conn.execute(sql_text("ALTER TABLE pretty_schedule_images ADD COLUMN module_id INTEGER"))
            rows = await conn.execute(sql_text("PRAGMA table_info(schedule_items)"))
            schedule_columns = {row[1] for row in rows.fetchall()}
            if schedule_columns and "setup" not in schedule_columns:
                await conn.execute(sql_text("ALTER TABLE schedule_items ADD COLUMN setup JSON DEFAULT '[]'"))
            rows = await conn.execute(sql_text("PRAGMA table_info(module_speakers)"))
            module_speaker_columns = {row[1] for row in rows.fetchall()}
            if module_speaker_columns and "flower_excluded" not in module_speaker_columns:
                await conn.execute(sql_text("ALTER TABLE module_speakers ADD COLUMN flower_excluded BOOLEAN DEFAULT 0"))
            rows = await conn.execute(sql_text("PRAGMA table_info(user_reminders)"))
            user_reminder_columns = {row[1] for row in rows.fetchall()}
            if user_reminder_columns and "sender_name" not in user_reminder_columns:
                await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN sender_name VARCHAR(255) DEFAULT ''"))
            if user_reminder_columns and "owner_user_id" not in user_reminder_columns:
                await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN owner_user_id INTEGER DEFAULT 0"))
            if user_reminder_columns and "batch_id" not in user_reminder_columns:
                await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN batch_id VARCHAR(64) DEFAULT ''"))
            rows = await conn.execute(sql_text("PRAGMA table_info(settings)"))
            settings_columns = {row[1] for row in rows.fetchall()}
            if "last_greeting_date" not in settings_columns:
                await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN last_greeting_date DATE"))
            if "performance_reminders_enabled" not in settings_columns:
                await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN performance_reminders_enabled BOOLEAN DEFAULT 1"))
            if "docs_reminders_enabled" not in settings_columns:
                await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN docs_reminders_enabled BOOLEAN DEFAULT 1"))
            if "flowers_reminders_enabled" not in settings_columns:
                await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN flowers_reminders_enabled BOOLEAN DEFAULT 1"))
            if "team_reminders_enabled" not in settings_columns:
                await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN team_reminders_enabled BOOLEAN DEFAULT 1"))
        elif dialect in {"postgresql", "postgres"}:
            await conn.execute(sql_text("ALTER TABLE access_grants ADD COLUMN IF NOT EXISTS permissions JSON DEFAULT '[]'"))
            await conn.execute(sql_text("ALTER TABLE pretty_schedule_images ADD COLUMN IF NOT EXISTS module_id INTEGER"))
            await conn.execute(sql_text("ALTER TABLE schedule_items ADD COLUMN IF NOT EXISTS setup JSON DEFAULT '[]'"))
            await conn.execute(sql_text("ALTER TABLE module_speakers ADD COLUMN IF NOT EXISTS flower_excluded BOOLEAN DEFAULT FALSE"))
            await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN IF NOT EXISTS sender_name VARCHAR(255) DEFAULT ''"))
            await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN IF NOT EXISTS owner_user_id INTEGER DEFAULT 0"))
            await conn.execute(sql_text("ALTER TABLE user_reminders ADD COLUMN IF NOT EXISTS batch_id VARCHAR(64) DEFAULT ''"))
            await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS last_greeting_date DATE"))
            await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS performance_reminders_enabled BOOLEAN DEFAULT TRUE"))
            await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS docs_reminders_enabled BOOLEAN DEFAULT TRUE"))
            await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS flowers_reminders_enabled BOOLEAN DEFAULT TRUE"))
            await conn.execute(sql_text("ALTER TABLE settings ADD COLUMN IF NOT EXISTS team_reminders_enabled BOOLEAN DEFAULT TRUE"))
...
async def main() -> None:
    ...
    async with SessionLocal() as session:
        ...
        for module_id in module_ids:
            module = await session.get(Module, module_id)
            await recreate_reminders_for_module(session, module_id, module.owner_user_id if module else 0)
        await session.commit()
    ...
    scheduler.add_job(send_photo_report_reminders, "cron", minute=0, args=[bot], id="send_photo_report_reminders")
    scheduler.start()
