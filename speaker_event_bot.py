...
async def program_module(callback: CallbackQuery) -> None:
    ...
    for image in images:
        ...
        if permission_allows(permissions, f"pretty_schedule_delete_ask:{image.id}"):
            kb.button(text=f"🗑 Удалить {image.day_label}", callback_data=f"pretty_schedule_delete_ask:{image.id}")
...
async def pretty_schedule_delete_ask(callback: CallbackQuery) -> None:
    ...
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await send_or_edit(callback, f"Удалить программу за {html_escape(image.day_label)}?", kb)
...
async def pretty_schedule_show(callback: CallbackQuery) -> None:
    ...
    kb.button(text="🏠 Главное меню", callback_data="menu")
    kb.adjust(1)
    await callback.message.answer_photo(FSInputFile(path), caption=f"🖼 {html_escape(image.day_label)}", reply_markup=kb.as_markup())
    await callback.answer()
        ...
        for rem in user_reminders:
            try:
                await bot.send_message(
                    ...
                    "\n".join([
                        "⏰ <b>Напоминание</b>",
                        f"От: <a href=\"tg://user?id={rem.sender_user_id}\">пользователь</a>",
                        "",
...
async def quick_reminder_when(message: Message, state: FSMContext) -> None:
    when = parse_reminder_datetime(message.text)
    if not when:
