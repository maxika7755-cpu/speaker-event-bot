        ...
        if speaker_names:
            for speaker_name in speaker_names:
                ...
                if not speaker:
                    ...
                    session.add(speaker)
                    await session.flush()
                ms = await ensure_module_speaker(session, module, speaker, topic=slot_data["title"])
                if speaker.gender.lower().startswith("жен"):
                    ms.flower_required = True
                new_item = ScheduleItem(**slot_data, speaker_id=speaker.id)
                session.add(new_item)
                new_items.append(new_item)
        else:
            new_item = ScheduleItem(**slot_data, speaker_id=None)
            session.add(new_item)
            new_items.append(new_item)
        await session.flush()
