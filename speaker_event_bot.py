...
def callback_permission_key(callback_data: str) -> str | None:
    ...
    if data == "reminders" or data.startswith(("reminders_page:", "recreate_reminders:", "user_reminder_delete")):
        return "reminders"
...
async def recreate_reminders_for_module(session: AsyncSession, module_id: int, user_id: int) -> None:
    ...
    for item in items:
        ...
        if settings.performance_reminders_enabled:
            session.add(Reminder(module_id=item.module_id, speaker_id=item.speaker_id, schedule_item_id=item.id, reminder_type="performance", remind_at=start_dt - timedelta(minutes=settings.performance_reminder_minutes), target_user_id=owner_user_id))
        if settings.docs_reminders_enabled and ms and ms.is_paid:
            session.add(Reminder(module_id=item.module_id, speaker_id=item.speaker_id, schedule_item_id=item.id, reminder_type="documents", remind_at=start_dt - timedelta(minutes=settings.docs_reminder_minutes), target_user_id=owner_user_id))
