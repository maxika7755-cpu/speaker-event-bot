...
def looks_like_speaker_name(text: str) -> bool:
    ...
    return capitalized >= 2 or infer_gender_from_name(text) == "женщина"
...
def kb_module(module_id: int) -> InlineKeyboardBuilder:
    ...
    buttons = [
        ...
        ("🗑 Удалить расписание", f"schedule_delete_menu:{module_id}"),
...
async def receive_schedule_file(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        async with SessionLocal() as session:
            module = await session.get(Module, module_id)
            parsed = await parse_schedule_with_ai(module, extracted, require_ai=True)
            parsed = await normalize_schedule_with_ai(module, parsed, extracted, require_ai=True)
        ...
        await message.answer(
            ...
            f"{html_escape(exc)}\n\n"
            "Проверьте OPENAI_API_KEY, модель и повторите загрузку файла.",
            reply_markup=kb_back(module_id).as_markup(),
        )
        return
    kb = InlineKeyboardBuilder()
...
def detect_excel_time_rows(ws: Any) -> list[tuple[int, str, str]]:
    ...
    for row in range(1, ws.max_row + 1):
        ...
        for col in range(1, ws.max_column + 1):
            value = clean_cell_value(ws.cell(row, col).value)
            tm = re.search(r"(\d{1,2}[:.]\d{2})\s*[-–]\s*(\d{1,2}[:.]\d{2})", value)
            if tm:
                row_start = tm.group(1).replace(".", ":")
                row_end = tm.group(2).replace(".", ":")
                break
            for single in re.findall(r"\b\d{1,2}[:.]\d{2}\b", value):
...
def validate_schedule_coverage(parsed: dict[str, Any], source_text: str) -> None:
    source_blocks = sum(1 for line in source_text.splitlines() if re.match(r"^\d{4}-\d{2}-\d{2}\s+\|\s+\d{1,2}:\d{2}-", line))
    parsed_items = sum(len(day.get("items", [])) for day in parsed.get("days", []))
    if source_blocks >= 6 and parsed_items < max(3, round(source_blocks * 0.35)):
        raise RuntimeError(
            f"ИИ вернул неполное расписание: распознано {parsed_items} слотов из примерно {source_blocks} ячеек Excel. "
            "Расписание не сохранено, чтобы не потерять данные. Попробуйте другую модель или повторите загрузку."
        )
...
async def access_add_save(message: Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if not existing:
            grant = AccessGrant(owner_user_id=message.from_user.id, username=username)
            session.add(grant)
