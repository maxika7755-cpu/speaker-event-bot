...
async def recreate_reminders_for_module(session: AsyncSession, module_id: int, user_id: int) -> None:
    ...
    for old in (await session.scalars(select(Reminder).where(Reminder.module_id == module_id, Reminder.is_sent == False))).all():
        if old.reminder_type in {"performance", "documents"} or old.target_user_id == user_id:
            await session.delete(old)
    items = (
        await session.scalars(
            select(ScheduleItem)
    ...
    for item in items:
        ...
        if module_speaker_needs_flowers(ms) and item.end_time:
...
async def reminder_recipients(session: AsyncSession, user_id: int, username: str | None, scope: str) -> list[int]:
    if scope == "self":
        return [user_id]
    owner_ids = await visible_owner_ids(session, user_id, username)
    owner_id = next((owner for owner in owner_ids if owner != user_id), user_id)
    recipients = {owner_id}
    grants = (
        await session.scalars(
