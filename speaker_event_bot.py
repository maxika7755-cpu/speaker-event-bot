...
async def receive_schedule_file(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    async with SessionLocal() as session:
        ...
        parsed = await normalize_schedule_with_ai(module, parsed, extracted)
        upload = UploadedFile(
            module_id=module_id,
            file_name=src_name,
...
async def normalize_schedule_with_ai(module: Module, parsed: dict[str, Any], source_text: str) -> dict[str, Any]:
