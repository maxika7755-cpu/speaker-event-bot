...
def callback_permission_key(callback_data: str) -> str | None:
    ...
    if data == "schedule_all" or data == "program" or data.startswith(("schedule_", "upload_schedule:", "pretty_schedule", "program_")):
        return "schedule"
...
async def upload_schedule(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        ...
        "PDF/XLSX тоже можно, но текстовый формат надежнее и дешевле по ИИ.",
        reply_markup=kb_back(module_id).as_markup(),
    )
    await callback.answer()
...
async def quick_reminder_send(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    text = (
        f"⏰ <b>Напоминание</b>\n"
        f"От: {actor_mention(callback.from_user)}\n\n"
        f"{html_escape(reminder_text)}"
    )
    sent = 0
    for recipient_id in recipients:
        try:
            ...
            log.warning("quick reminder send failed to %s: %s", recipient_id, exc)
    await state.clear()
    permissions = await effective_permissions(callback.from_user.id, callback.from_user.username)
    await send_or_edit(callback, f"✅ Напоминание отправлено. Получателей: {sent}.", kb_main(permissions))
